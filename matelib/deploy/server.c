...
int recibir_codigo(int socketOrigen)
{
	int cod_op;
	if(recv(socketOrigen, &cod_op, sizeof(int), MSG_WAITALL) != 0){
...
void* recibir_buffer(int* size, int socketOrigen)
{
	void * buffer;

	recv(socketOrigen, size, sizeof(int), MSG_WAITALL);
	buffer = malloc(*size);
	recv(socketOrigen, buffer, *size, MSG_WAITALL);

	return buffer;
}
void enviar_datos(void *dato,int tamDato,int socketDestino)
{
	t_paquete* paquete = malloc(sizeof(t_paquete));

	paquete->buffer = malloc(sizeof(t_buffer));
	paquete->buffer->size = tamDato;
	paquete->buffer->stream = malloc(paquete->buffer->size);
	memcpy(paquete->buffer->stream, dato, paquete->buffer->size);

	int bytes = paquete->buffer->size + 2*sizeof(int);

	void* a_enviar = serializar_paquete(paquete, bytes);

	send(socketDestino, a_enviar, bytes, 0);
...
void *recibir_datos(int socketOrigen)
{
	int size;
	char* buffer = recibir_buffer(&size, socketOrigen);
...
void enviar_codigo(int codigo,int socketDestino){

	send(socketDestino, &codigo, sizeof(int), 0);
	return;
}


void enviar_codigo_y_datos(int codigo, void *dato,int tamDato, int socketDestino)
{
	t_paquete* paquete = malloc(sizeof(t_paquete));
